...
func save_game_state(setting):
	...
	if save_path == null:
		print("Failed to find settings path aborting save")
		return
...
func load_game_state(setting):
	...
	print("Failed to load " + save_path)
	return setting
...
func type_to_file_path(typename):
	if not typename.has_method("print_name"):
		print("This type does not have a print_name() function please implement one")
		return null
